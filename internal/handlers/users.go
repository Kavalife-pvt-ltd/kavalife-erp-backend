...
func AllUsers(c context.Context) ([]models.User, error) {
	...
	var err error
	rows, err := db.DB.Query(c, "select id, username, created_at, role, department, phone_num FROM public.users")
	if err != nil {
	...
	for rows.Next() {
		var p models.User
		err := rows.Scan(&p.ID, &p.Username, &p.Created_at, &p.Role, &p.Department, &p.Phone_Num)
		if err != nil {
